 ...
 * They will never show the same message twice in a row.
 *
 * PAGE-BOTTOM POP-UPS
 * ===================
 * Pop-ups in `pageBottomPopups` trigger once when a user scrolls
 * to the bottom of a specific claim page (e.g. /page-1).
 *
 * HOW TO ADD A NEW RANDOM POP-UP:
 *   1. Add a new object to the `popupMessages` array
 *   2. Give it a unique `id`, `title`, and `content`
 *
 * HOW TO ADD A PAGE-BOTTOM POP-UP:
 *   1. Add a new entry to `pageBottomPopups` with the route as key
 *   2. The pop-up will show every time that page's bottom is reached
 */
...
/** Random pop-ups — shown on a timer, never the same one twice in a row */
