...
## PAGE-BOTTOM POP-UPS (Scroll-triggered per page)

This feature shows a specific pop-up every time a user scrolls to the bottom of a claim page. Each page (1–5) has its own unique joke/message.

### New file: `src/components/PageBottomPopup.tsx`

**Action**: Create this file and paste the entire contents from the project.

**What it does**: Detects when the user scrolls within 100px of the page bottom. If the current route has a matching entry in `pagePopups`, it shows that pop-up. It triggers once per page visit (resets when navigating away and back).

### Modified file: `src/data/popupMessages.ts`

**What changed**: Added a new `PagePopup` interface and `pagePopups` array alongside the existing `popupMessages`.

**ADD** these new exports to `src/data/popupMessages.ts` (above the existing `popupMessages` array):
...
To add more page pop-ups, copy the template and set `triggerPage` to the route path.
...
**ADD this line** right after `<RandomPopup />`, before the closing `</SidebarProvider>`:
...
### Also changed: No-repeat random pop-ups

In `src/components/RandomPopup.tsx`, the `showRandomPopup` function was updated so the same random pop-up never shows twice in a row. **Replace** the entire `showRandomPopup` callback with the version from the project.
